...
import com.my.security.bean.LocalCancelLog;
...
public class AccountServiceImpl implements AccountService {
	...
	public void updateBank1Account(AccountInfo obj) {
		...
		lcl.setTxNo(xid);
		LocalCancelLog lcanl = new LocalCancelLog();
		lcanl.setTxNo(xid);
	...
	@Transactional
	public void commit(AccountInfo obj) {
		// 获取全局事务XID
		String xid = HmilyTransactionContextLocal.getInstance().get().getTransId();
		log.info("bank1 commit try 方法开始", xid);

	}

	@Transactional
	public void rollback(AccountInfo obj) {
		// 获取全局事务XID
		String xid = HmilyTransactionContextLocal.getInstance().get().getTransId();
		log.info("bank1 rollback try 方法开始", xid);
