 ...
 * 1、请求头添加token，基于sesion获取方式
 * 2、基于session token到期后刷新token
 */
@Component
@Slf4j
public class SessionAnddTokenFilter extends ZuulFilter {
		...
		if (token != null) {
			...
			if (token.isExprie()) {
				...
				try {
					...
					ResponseEntity<TokenInfo> response = restTemplate.exchange(checkTokenEndpointUrl, HttpMethod.POST,
